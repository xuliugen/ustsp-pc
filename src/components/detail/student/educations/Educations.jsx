...
export default class Educations extends React.Component {
  render() {
    const educationItem = this.props.educations.map((item, idx) => {
      return (
        <div key={idx} styleName="education-item">
          ...
          <span styleName="edu-duration">{moment(item.startTime).format('YYYY-MM-DD')}  / {moment(item.endTime).format('YYYY-MM-DD')}</span>
